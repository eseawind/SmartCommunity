...
public class PageInterceptor implements Interceptor {
	private org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(PageInterceptor.class);
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof Map) {
			Map map = (Map) obj;
			RepairExample example = (RepairExample)map.get("example");
			logger.info("getCriteria  "+example.getOredCriteria().get(0).getCriteria().get(0).getValue());
			Object pageobj = ((Map) obj).get("page");
			if (pageobj != null) {
		...
		//		logger.info("pageno" + page.getPageNo());
				this.setTotalRecord(map, mappedStatement, connection);
				// 获取分页Sql语句
				String pageSql = sql;
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecord(Map map, MappedStatement mappedStatement,
		...
		// 通过BoundSql获取对应的参数映射
		logger.info("count bound sql " + countSql);
		List<ParameterMapping> parameterMappings = boundSql
				.getParameterMappings();

	//	logger.info("parameters " + parameterMappings.get(0).getProperty());
		...
		ParameterHandler parameterHandler = new DefaultParameterHandler(
				mappedStatement, map, countBoundSql);
		...
		try {
			...
			if (rs.next()) {
				...
				((Page<?>) map.get("page")).setTotalRecord(totalRecord);
			}
...
//		sql = "select count(*) from repair  left outer join repairimages on repair.id = repairimages.repairid where processedstate = 2";
		return sql;
	}
